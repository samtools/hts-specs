...
i. If $M$ is power of 2, code remainder as binary format. So $log_{2}(M)$ bits are needed. (Rice code)  

ii. If $M$ is not a power of 2, set $b=\lceil log_{2}(M) \rceil$

1. If $r < 2^{b}-M$ code $r$ as plain binary using $b-1$ bits.

2. If $r \ge 2^{b}$ code the number $r+2^{b}$ in plain binary representation 
using $b$ bits.
...
b. -- if $n \ge 2^{k}$ , then determine $i$ such that $2^{i+k-1} \le n < 2^{i+k}$
