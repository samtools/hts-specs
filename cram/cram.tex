...
\title{CRAM format specification}\maketitle
...
\textit{date: 12th of February 2014}

\textit{license: }\textit{\emph{Apache 2.0}}

\textit{by: }\textit{\emph{EMBL-EBI}}

\textit{contacts: }\textit{\emph{cram-dev@ebi.ac.uk}}


\newpage\tableofcontents\newpage
\section{\textbf{Overview}}

This specification describes the CRAM 2.0 format. 
...
Boolean is either `true' (numerical 1) or `false' (numerical 0).
...
\begin{tabular}{|>{\raggedright}p{85pt}|>{\raggedright}p{104pt}|>{\raggedright}p{59pt}|>{\raggedright}p{88pt}|>{\raggedright}p{70pt}|}
\hline
\textbf{operation order} & \textbf{buffer state before} & \textbf{written bits} & \textbf{buffer 
state after} & \textbf{issued bytes}\tabularnewline
\hline
...
\texttt{>}echo \texttt{"}obase=16; ibase=2; 00000111\texttt{"} \textbar{} bc

7

\texttt{>}echo \texttt{"}obase=16; ibase=2; 01110000\texttt{"} \textbar{} bc

70

And the whole bit sequence: 

\texttt{>}echo \texttt{"}obase=2; ibase=16; B070\texttt{"} \textbar{} bc
...
\subsubsection*{LTF-8 long or (ltf8)}
...
\begin{tabular}{|>{\raggedright}p{79pt}|>{\raggedright}p{41pt}|>{\raggedright}p{41pt}|>{\raggedright}p{41pt}|>{\raggedright}p{41pt}|>{\raggedright}p{41pt}|>{\raggedright}p{41pt}|>{\raggedright}p{41pt}|}
\hline
...
\begin{tabular}{|>{\raggedright}p{112pt}|>{\raggedright}p{15pt}|>{\raggedright}p{157pt}|>{\raggedright}p{130pt}|}
\hline
...
%%\begin{figure}[htbp]
\includegraphics[width=356pt, height=31pt, keepaspectratio=true]{CRAMFIleFormat2-1-fig001.png}
%%\caption{This should be the caption for \texttt{CRAMFIleFormat2-1-fig001.png}.}
%%\end{figure}
...
%%\begin{figure}[htbp]
\includegraphics[width=354pt, height=103pt, keepaspectratio=true]{CRAMFIleFormat2-1-fig002.png}
%%\caption{This should be the caption for \texttt{CRAMFIleFormat2-1-fig002.png}.}
%%\end{figure}
...
%%\begin{figure}[htbp]
\includegraphics[width=356pt, height=154pt, keepaspectratio=true]{CRAMFIleFormat2-1-fig003.png}
%%\caption{This should be the caption for \texttt{CRAMFIleFormat2-1-fig003.png}.}
%%\end{figure}
...
%%\begin{figure}[htbp]
\includegraphics[width=354pt, height=103pt, keepaspectratio=true]{CRAMFIleFormat2-1-fig004.png}
%%\caption{This should be the caption for \texttt{CRAMFIleFormat2-1-fig004.png}.}
%%\end{figure}
...
%%\begin{figure}[htbp]
\includegraphics[width=374pt, height=137pt, keepaspectratio=true]{CRAMFIleFormat2-1-fig005.png}
%%\caption{This should be the caption for \texttt{CRAMFIleFormat2-1-fig005.png}.}
%%\end{figure}
...
%%\begin{figure}[htbp]
\includegraphics[width=392pt, height=149pt, keepaspectratio=true]{CRAMFIleFormat2-1-fig006.png}
%%\caption{This should be the caption for \texttt{CRAMFIleFormat2-1-fig006.png}.}
%%\end{figure}
...
$\bullet$ The SQ:MD5 checksum is required unless the reference sequence has been embedded 
into the file.

$\bullet$ At least one RG record is required.

$\bullet$ The HD:SO sort order is always POS.
...
\begin{tabular}{|>{\raggedright}p{111pt}|>{\raggedright}p{87pt}|>{\raggedright}p{101pt}|>{\raggedright}p{119pt}|}
\hline
...
\begin{tabular}{|>{\raggedright}p{100pt}|>{\raggedright}p{100pt}|>{\raggedright}p{100pt}|>{\raggedright}p{119pt}|}
\hline
\textbf{Key} & \textbf{Value data type} & \textbf{Name} & \textbf{Value}\tabularnewline
\hline
...
Let L\textsubscript{i}=\{T\textsubscript{i0}, T\textsubscript{i1}, ..., T\textsubscript{ix}\} 
be sorted list of all tag ids for a record R\textsubscript{i}, where i is the sequential 
record index and T\textsubscript{ij} denotes j-th tag id in the record. We recommend 
alphabetical sort order. The list of unique L\textsubscript{i} is assigned sequential 
integer numbers starting with 0. These integer numbers represent the TL data series. 
The sorted list of unique L\textsubscript{i} is stored as the TD value in the preservation 
map. Using TD, an integer from the TL data series can be mapped back into a list 
of tag ids. 

The TD is written as byte array consisting of L\textsubscript{i} values separated 
with \textbackslash{}0. Each L\textsubscript{i} value is written as a sequence 
...
\textbf{KeyValue data typeNameValue}
...
%%\begin{figure}[htbp]
\includegraphics[width=451pt, height=350pt, keepaspectratio=true]{CRAMFIleFormat2-1-fig007.png}
%%\caption{This should be the caption for \texttt{CRAMFIleFormat2-1-fig007.png}.}
%%\end{figure}
...
\begin{tabular}{|>{\raggedright}p{108pt}|>{\raggedright}p{53pt}|>{\raggedright}p{153pt}|>{\raggedright}p{105pt}|}
\hline
\textbf{hex bytes} & \textbf{data type} & \textbf{decimal value} & \textbf{field 
name}\tabularnewline
\hline
C\textit{ontainer header} &  &  & \tabularnewline
\hline
...
\hline
C\textit{ompression header block} &  &  & \tabularnewline
\hline
...
\hline
C\textit{ompression header} &  &  & \tabularnewline
\hline
...
1. M5 (sequence MD5 checksum) field of @SQ sequence record in the BAM header is 
...
\texttt{>} samtools faidx human\_g1k\_v37.fasta 1 \textbar{} grep -v '\textasciicircum{}\texttt{>}' 
\textbar{} tr -d '\textbackslash{}n' \textbar{} tr a-z A-Z \textbar{} md5sum -

1b22b98cdeb4a9304cb5d48026a85128  -

\texttt{>} samtools faidx human\_g1k\_v37.fasta 1:10-20 \textbar{} grep -v '\textasciicircum{}\texttt{>}' 
\textbar{} tr -d '\textbackslash{}n' \textbar{} tr a-z A-Z \textbar{} md5sum -
...
2. All CRAM reader implementations are expected to check for reference MD5 checksums 
...
3. In some cases reads may be mapped beyond the reference sequence. All out of 
range reference bases are all assumed to be `N'. 

4. MD5 checksum bytes in slice header should be ignored for unmapped or multiref 
slices. 
...
4. Separate the integer into the highest power of 2 it contains (2N) and the remaining 
...
1. Read and count 0s from the stream until you reach the first 1. Call this count 
of zeroes N.

2. Considering the one that was reached to be the first digit of the integer, with 
...
1. Fix the parameter \textit{M} to an integer value.

2. For \textit{N}, the number to be encoded, find

a. quotient = \textit{q} = int[\textit{N}/\textit{M}]

b. remainder = \textit{r} = \textit{N} modulo \textit{M}

3. Generate Codeword

a. The Code format : \texttt{<}Quotient Code\texttt{>}\texttt{<}Remainder Code\texttt{>}, 
where

b. Quotient Code (in unary coding)

i. Write a \textit{q}-length string of 1 bits

ii. Write a 0 bit

c. Remainder Code (in truncated binary encoding)

i. If \textit{M} is power of 2, code remainder as binary format. So log\textsubscript{2} 
(M) bits are needed. (Rice code)  

ii. If \textit{M} is not a power of 2, set b=[log\textsubscript{2}(M)]

1. If r\texttt{<}2\textsuperscript{b}-M code \textit{r} as plain binary using \textit{b}-1 
bits.

2. If r
\includegraphics[width=8pt, height=15pt, keepaspectratio=true]{CRAMFIleFormat2-1-fig008.png}
2\textsuperscript{b} code the number r+2\textsuperscript{b} in plain binary representation 
using \textit{b} bits.
...
Subexponential coding is parametrized by a non-nengative integer \textit{k}. The 
main feature of the subexponential code is its length. For integers \textit{n \texttt{<} 
2k+1} the code length increases linearly with \textit{n}, but for larger \textit{n}, 
...
a. - if n \texttt{<} 2\textsuperscript{k} , then i = 0. 

b. - if n $\ge$ 2\textsuperscript{k} , then determine i such that 2\textsuperscript{i+k-1} 
$\le$ n \texttt{<} 2\textsuperscript{i+k} .

2. Form the prefix of i 1s.

3. Insert the separator 0.

4. Form the tail: express the value of (n - 2\textsuperscript{i+k-1}) as a 
(i + k - 1)-bit binary number if i \texttt{>} 0 and n as a k-bit binary number 
otherwise.
...
b. 2\textsuperscript{i+k-1}, otherwise

3. Skip the next 0 (separator).

4. Compute the length of the tail, c\textsubscript{tail} as

a. k, if i = 0

b. k + i - 1, if i $\ge$ 1

5. The next c\textsubscript{tail} bits are the tail. Form a run of 0s of length 
