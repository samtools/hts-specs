...
\tikzstyle{encodingArrow} = [thick, decoration={markings, mark=at position
   1 with {\arrow[semithick]{open triangle 60}}},
   double distance=3.0pt, shorten >= 5.0pt,
   preaction = {decorate},
   postaction = {draw, line width=1.4pt, white, shorten >= 5.0pt}]
...
\begin{pgfonlayer}{background}
   ...
   \path (a1.east |- a1.south)+(0.8,-0.3) node[texto]
      {\scriptsize\textit{Bit Stream}};
   \path (a3.east |- a3.south)+(0.8,-0.3) node[texto]
      {\scriptsize\textit{Byte Streams}};
\end{pgfonlayer}}

\path (a1.south)+(10.0,-1.0) \encodedblock{7}{Core Data Block};
\path (p7.south)+(0.0,-2.5) \encodedblock{8}{External Data Block};
\path (p8.south)+(0.0,-1.0) \encodedblock{9}{External Data Block};
\path (p9.south)+(0.0,-1.0) \encodedblock{10}{External Data Block...};

\blocks{p7}{p8}{p9}{p10}

\draw[encodingArrow] (p1) + (1.7, -0.0) to (p7);
\draw[encodingArrow] (p3) + (1.7, 0.0) to (p8);
\draw[encodingArrow] (p4) + (1.7, 0.0) to (p9);
\draw[encodingArrow] (p5) + (1.7, 0.0) to (p10);

\filldraw[draw=black, fill=white!10, drop shadow] (1.5, -1.5) ellipse (2.0 and 0.5);
\filldraw[draw=black, fill=white!10, drop shadow] (1.5, -4.55) ellipse (1.8 and 0.5);
\filldraw[draw=black, fill=white!10, drop shadow] (1.5, -6.1) ellipse (1.8 and 0.5);
\filldraw[draw=black, fill=white!10, drop shadow] (1.5, -7.6) ellipse (1.8 and 0.5);

\node [right=of p1, scale=0.7, text centered, text width=14em] {Core (Bit) Encodings (Huffman, RANS, etc.)};
\node [right=of p3, scale=0.7, text centered, text width=14em] {External (Byte) Encodings};
\node [right=of p4, scale=0.7, text centered, text width=14em] {External (Byte)  Encodings};
\node [right=of p5, scale=0.7, text centered, text width=14em] {External (Byte)  Encodings};
